 ...
 * availability state.
 *
 * Scope note. This module owns only the visibility/opacity decision; the
 * `LPS → view plane → viewport → panel content → sheet mm` projection lives in
 * `patient-projection.ts` and consumes the transforms authored by
 * `view-engine`/`medical-engine` (ADR-014 OD-5, ratified 2026-09-23). The
 * out-of-plane distance is computed there from the physical LPS plane, never
 * from the projective `z`.
 *
 * Ratified rules:
