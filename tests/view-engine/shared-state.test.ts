/**
 * NuClear P4.3 — shared-state group tests (ADR-011 §3 + binding addendum).
 *
 * Pure Node: no DOM, WebGL or Cornerstone. Proves the
 * `private holder → atomic replacement → new projection → frozen published DTO`
 * contract: holder / `PreparedViewId` / `ViewSlot` identity stays stable, the
 * `PreparedView` projection is regenerated, attached views observe the new
 * frozen pair by object identity (never a clone), and every invalid replacement
 * is refused with the typed `WorkspaceError` while leaving the holder and every
 * bound view unchanged.
 *
 * The test file statically imports the shared workspace fixtures (which
 * register the `.js`→`.ts` resolve hook) before dynamically importing the real
 * product sources by value.
 */
...
const { SharedStateGroupRegistry, SharedStateError } = sharedStateModule;
const { assemblePreparedView, PreparedViewError, PreparedViewRegistry } = preparedViewModule;

const GROUP_A = 'shared-group-a' as SharedStateGroupId;
const GROUP_B = 'shared-group-b' as SharedStateGroupId;
const UNKNOWN_GROUP = 'shared-group-missing' as SharedStateGroupId;
const PET_PREPARED = 'prepared-view-shared-pet' as PreparedViewId;
const FUSION_PREPARED = 'prepared-view-shared-fusion' as PreparedViewId;
const UNKNOWN_PREPARED = 'prepared-view-shared-missing' as PreparedViewId;

type Workspace = InstanceType<typeof ImagingWorkspace>;
type Pair = { spatial: SpatialState; camera: CameraState };
...
function registerPetView(workspace: Workspace, id: PreparedViewId = PET_PREPARED): PreparedView {
...
function registerFusionView(workspace: Workspace, id: PreparedViewId = FUSION_PREPARED): PreparedView {
  const view = assemblePreparedView({
    preparedViewId: id,
...
describe('NuClear P4.3 — shared-state groups (ADR-011 §3 + addendum)', () => {
