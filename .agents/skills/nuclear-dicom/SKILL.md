...
3. **Decay Correction & Radionuclide Info**:
   - Verify `DecayCorrection` (0054,1102) is `START` or `ADMIN`.
   - Radionuclide half-life $T_{1/2}$ for $^{18}\text{F} = 6586.2\text{ s}$ (from `RadionuclideHalfLife` 0018,1075).
   - Injected dose: `RadionuclideTotalDose` (0018,1074) in Becquerels.
   - Elapsed time $\Delta t = \text{SeriesTime} - \text{RadiopharmaceuticalStartTime}$.
