...
- **THEN** the agent MUST run and verify the verification gates:
  1. **Changelog Gate**: Any notable feature, architectural refactor, fix, or contract addition MUST have a documented entry in `CHANGELOG.md` under `## [Unreleased]` matching the Keep a Changelog format before committing.
  2. **File Length Gate**: Inspect all created or modified source files. If any source file exceeds **300 lines** (excluding pure data tables or static mockups/palettes), the commit/completion is blocked until modular decomposition is performed.
  3. `npm run typecheck` — 0 errors across all configured workspaces.
  4. `npm test` and Python tests — all configured, discovered tests pass.
  5. `npm run build` — production bundles compile cleanly once a production build exists.
...
- **Changelog Maintenance**: Update `CHANGELOG.md` under `## [Unreleased]` alongside the source code changes for any notable commit.
