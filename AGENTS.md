...
6. **MedCanvas as Oracle**: The legacy MedCanvas repository serves as the clinical oracle, golden master, and source of verified algorithms (SUV calibration, geometry verification, persistent-surface principles, per-row radiometric isolation). It is NOT a runtime dependency.
...
- Strategy: Greenfield repository structured on the v3 Architecture Blueprint ([NUCLEAR_ARCHITECTURE_V3.md](docs/NUCLEAR_ARCHITECTURE_V3.md)), executing the phased implementation plan:
  ...
  - **Fase 6**: Legacy transition adapters (reading MedCanvas `.mcv` projects).
  - **Fase 7**: UI mockup with fake surfaces.
  - **Fase 8**: Production UI, mouse interaction layer, and desktop shell.
...
- Commands: [`.opencode/commands/`](.opencode/commands/) — `/phase <0–8> [goal]` drives one milestone; `/verify [scope]` reports the real verification state; `/review [scope]` runs the independent audit.
...
- [`nuclear-testing`](.agents/skills/nuclear-testing/SKILL.md): Headless verification pipeline, typechecking, golden test comparison against legacy oracle, production builds.
