...
## Current Baseline — Phase 5 Complete (P5.0–P5.8)
- Strategy: Greenfield repository structured on the v3 Architecture Blueprint ([NUCLEAR_ARCHITECTURE_V3.md](docs/NUCLEAR_ARCHITECTURE_V3.md)), executing the phased implementation plan:
  ...
  - **Fase 5**: Figure engine & publication export (**P5.0–P5.8 complete (2026-09-23)**; see `docs/plans/PHASE_5_FIGURE_ENGINE_PLAN.md`, `docs/agentlog/phase-5.md`, ADR-014/ADR-015/ADR-016). `@nuclear/figure-engine` delivers headless editorial composition (physical mm, OD-1 framing, OD-2 placement, OD-4/OD-5 annotation policy/projection), fail-closed `PublicationRenderRequest` assembly accepted by the Fase-1 oracle, the `PublicationRendererPort` (fake + real-harness 1890×1890 evidence), a pure no-resampling sheet compositor with `EncoderPort` and deterministic `node:zlib` PNG/TIFF writers, and a hybrid vector PDF (`pdf-lib` + embedded Inter OFL; panel rasters as image XObjects, native text/line/rect/ellipse/polygon vectors, deterministic metadata and content-hash `/ID`). ADR-016 maps `FigureSheet` editorial content to native vectors (OD-7a paint order, OD-7j typed refusals). Phase 5 is **closed**; it may be **reopened additively** for tracked follow-ups: (a) aperture-inside-panel placement, (b) PNG/TIFF text/vector rasterization, (c) residual OD-7k patient-space ROI/text orientation, (d) flattened-raster physical-resolution metadata — none silently done and none assumed by later phases. Evidence is software WebGL 2 (SwiftShader); hardware-GPU, a true production bundle and real-dataset pixel-tolerance remain `NOT YET APPLICABLE`.
  - **Fase 6**: UI mockup with fake surfaces (not started).
